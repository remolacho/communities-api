...
RSpec.describe Fines::Statuses::Legal::Pending do
  include_context 'legal_status_fines_stuff'

  context 'when getting list of allowed statuses for pending legal fine' do
    before do
      fine.update(status: status_pending)
    end
    ...
    it 'creator can only see pending status' do
      user_role_creator
    ...
    it 'owner can only see pending status' do
      user_role_owner
