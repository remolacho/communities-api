...
RSpec.describe Fines::Statuses::Legal::Rejected do
  include_context 'legal_status_fines_stuff'

  context 'when getting list of allowed statuses for rejected legal fine' do
    before do
      fine.update(status: status_rejected)
    end
    ...
    it 'creator can only see rejected status' do
      user_role_creator
    ...
    it 'owner can only see rejected status' do
      user_role_owner
