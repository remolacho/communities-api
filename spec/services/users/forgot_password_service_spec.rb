...
RSpec.describe Users::ForgotPasswordService do
  ...
  context 'When 1 user want recover password because it forgot' do
    it 'return error because the email is empty' do
      service = described_class.new(email: '')
      expect { service.call }.to raise_error(ArgumentError)
    end

    it 'return error because the email not found' do
      service = described_class.new(email: 'testError@error.com')
      expect { service.call }.to raise_error(ActiveRecord::RecordNotFound)
    end

    it 'return error because the user is inactive' do
    ...
    it 'return success with reset_password_key' do
      user_enterprise
